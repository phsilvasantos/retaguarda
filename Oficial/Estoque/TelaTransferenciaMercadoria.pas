...
type
  TFormTelaTransferencia = class(TFormTelaGeralTEMPLATE)
    ...
    mProdutoCodigoProduto: TIntegerField;
    mProdutoNomeProduto: TStringField;
    mProdutoQuantidade: TFloatField;
    dsmProduto: TDataSource;
    gridCodBarra: TDBGrid;
    btnGerar: TRxSpeedButton;
    mProdutoCodigo_Barra: TStringField;
    mProdutoReferencia: TStringField;
    mProdutoPreco_Custo: TFloatField;
    procedure FormCreate(Sender: TObject);
    procedure SQLEmpresaRemetenteCalcFields(DataSet: TDataSet);
    procedure IncluiProduto;
    procedure IncluiMProduto;
...
begin
  inherited;
  btReceberTransferencia.Enabled := (SQLTranferenciaTRFECRECEBIDO.AsString = 'N') and (SQLTranferenciaEMPRICOD.AsInteger <> EmpresaCorrente) and (PageControl.ActivePage = TabTransfRecebida);
...
begin
  ...
  begin
    GravaMovimentoEstoqueSimples(DM.SQLProduto, DM.SQLProdutoFilho, DM.SQLProdutoSaldo, SQLTranferenciaTRFEIEMPRDEST.AsInteger, SQLTranferenciaItemPRODICOD.AsInteger, DM.SQLConfigGeral.FieldByName('OPESICODTRANSFENTR').AsInteger, SQLTranferenciaItemTRITN3QTDEENVIADA.AsFloat, 'E', FormatDateTime('mm/dd/yyyy', Now), '0', 'TRANSFERENCIA', SQLTranferenciaItemTRFEA13ID.AsString, '');
...
procedure TFormTelaTransferencia.IncluiMProduto;
begin
  if (StrToFloat(edQtdeProduto.Text) = 0) then
  begin
    edQtdeProduto.SetFocus;
  ...
  if not mProduto.Active then
    mProduto.Active := True;
  try
    begin
      mProdutoQuantidade.AsFloat := StrToFloat(edQtdeProduto.Text);
      mProduto.Post;
    end;

     // Totais da Transf Atual
    EdtCustoTotal.Value := EdtCustoTotal.Value + edtCusto.Value;
    EdtQtdeTotalItens.Value := EdtQtdeTotalItens.Value + mProdutoQuantidade.Value;
...
procedure TFormTelaTransferencia.chkUsaCodBarraExit(Sender: TObject);
begin
  inherited;
  if chkUsaCodBarra.Checked then
  begin
    if mProduto.RecordCount > 0 then
      mProduto.EmptyDataSet;
    btnGerar.Enabled := chkUsaCodBarra.Checked;
    gridCodBarra.Visible := chkUsaCodBarra.Checked;
  end
  else
  begin
    btnGerar.Enabled := chkUsaCodBarra.Checked;
...
procedure TFormTelaTransferencia.btnGerarClick(Sender: TObject);
begin
  inherited;
  mProduto.First;
  if not (mProduto.IsEmpty) then
  begin
    ...
    begin
      ...
      tblTempTranferenciaProdutoNome.AsString := mProdutoNomeProduto.AsString;
      tblTempTranferenciaProdutoEstoque.AsFloat := StrToFloat(SQLLocate('PRODUTOSALDO', 'PRODICOD', 'PSLDN3QTDE', tblTempTranferenciaProdutoCod.AsString));
      tblTempTranferenciaProdutoReferencia.AsString := mProdutoReferencia.AsString;
      tblTempTranferenciaProdutoValorCusto.AsFloat := mProdutoPreco_Custo.Value;
      tblTempTranferencia.Post;
      mProduto.Next;
    end;
    mProduto.EmptyDataSet;
